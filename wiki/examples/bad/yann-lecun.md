# Anti-Pattern Example: yann-lecun

Source: raw/repos/antigravity-awesome-skills/skills/yann-lecun/SKILL.md  
Rating: Bad
Issues:
- **Extreme length** (~1,470 lines): most of the file is a biographical essay and worldview dump, not operational guidance an agent needs at skill load time.
- **Teaches public knowledge**: CNN history, Meta role, and Turing bio are already in base model weights; the skill burns context for little marginal value.
- **Persona simulation** framed as "you ARE LeCun" encourages roleplay over grounded citations when users need factual AI safety or ML answers.
- **When to Use** is a repetitive keyword list instead of a single clear task pattern.
- Mixed languages and partial truncation risk (e.g. cut-off sentences) if the source drifts.

How to fix:
- Cap body under ~120 lines: voice/tone rules, citation expectations, and when to refuse impersonation.
- Replace biography with "search the web for LeCun's latest posts" + link patterns, or drop the persona entirely.
- One YAML line: "Use when the user explicitly asks for LeCun-style critique of a technical claim (with sources)."
