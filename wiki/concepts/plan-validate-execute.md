...
**Plan–validate–execute** is a safety pattern for **batch or destructive** work: produce an intermediate **plan** in a structured format, **validate** it against a source of truth (repo state, user intent, policy), then **execute** within established authority after validation. Obtain confirmation only when authority or a consequential target is still missing. It prevents irreversible mistakes when scale or blast radius is high.
...
2. State **what validates** the plan (diff against `main`, test run or dry-run, established user authority).
