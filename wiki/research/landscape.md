...
The “skill” layer for coding agents sits between the base model and project-specific rules: packaged procedures, bundled scripts, and domain playbooks that hosts discover and load on demand. This article compares the high-signal repositories in the local Skill Factory corpus—by format, scale, host compatibility, and engineering trade-offs—not to crown a winner but to show where each system optimizes. The corpus now tracks **154 source entries (153 distinct canonical repositories)** including `github-ranking`, SkillsBench, and Gemini Skills; a 100-repo sparse expansion on 2026-05-25 showed that skills increasingly live inside product repos, host folders, subagent configs, MCP configs, and command/hook systems rather than only dedicated skill packs.

## September 2026 refresh

The existing detailed profiles below are historical April/May snapshots; their
star counts, file counts, source names, and paths are not re-certified by a HEAD
check. Current revisions and sampled paths are in the
[September coverage dataset](../../raw/datasets/source-refresh-2026-09-05.json).

New retained sources and their authoring contribution:

- **agentskills/agentskills** — Official format plus authoring and evaluation guidance.
- **vercel-labs/skills** — Whole-skill update hashes and installation boundary regression tests.
- **NVIDIA/SkillSpector** — Bundled execution-surface inspection and incomplete-scan gates.
- **EverMind-AI/SkillCorpus** — Curated multi-source skill retrieval with ranking metrics and license gates.
- **rpamis/comet** — Named eval treatments, package snapshots, and explicit reliability estimators.
- **NVIDIA/SkillEvaluator** — Tiered evaluation, trusted negative-control evidence, and runtime isolation tests.
- **langchain-ai/skills-benchmarks** — No-skill and multi-skill treatment definitions; packaging and interference experiments.

All new-source evidence here is source/test-definition inspection, not executed
behavioral proof. See [September synthesis](skill-evolution-2026-09.md) for tradeoffs
and [the update report](../queries/monthly-update-2026-09.md) for selection decisions.
...
- **Source-level signal:** The repository implements a provider translation shim. This review did not execute it or verify its advertised model coverage.
...
- **Notable pattern:** Protocol translation may reduce migration work while keeping skill prose stable. Compatibility still requires executed tests for each model, host, tool policy, and version.
